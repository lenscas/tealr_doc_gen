...
pub(super) fn run_and_write(
    ...
    let page_path: PathBuf = match &instance_setter.page {
        TypeOrPage::Type(x) => match x.type_members.type_name() {
            ...
            .unwrap_or_else(|| write_path.join(&x.type_name)),
            tealr::Type::Function(_)
            | tealr::Type::Map(_)
            | tealr::Type::Or(_)
            | tealr::Type::Array(_)
            | tealr::Type::Tuple(_)
            | tealr::Type::Variadic(_) => write_path.join(x.type_name.clone()),
        },
        TypeOrPage::IndexPage(x) => write_path.join(&x.type_name),
        TypeOrPage::CustomPage(x) => write_path.join(&x.name),
    }
    .with_extension("html");
